...
class Sitemap {
  public static function generate($file, array $locs, $domain = null) {
    if (!$domain) {
    ...
    file_put_contents($file, '<?xml version="1.0" encoding="UTF-8"?>'
      ...
      . PHP_EOL . '<url>'
      . PHP_EOL . '  <loc>' . config('common.proto') . '://' . $domain . '</loc>'
      . PHP_EOL . '  <lastmod>' . gmdate('Y-m-d\TH:i:sP') . '</lastmod>'
      . PHP_EOL . '  <priority>1.0</priority>'
      . PHP_EOL . '</url>'
      . PHP_EOL . implode('',
        array_map(function ($loc) use ($domain) {
          return
            PHP_EOL . '<url>'
            . PHP_EOL . '  <loc>' . config('common.proto') . '://' . $domain . $loc['url'] . '</loc>'
